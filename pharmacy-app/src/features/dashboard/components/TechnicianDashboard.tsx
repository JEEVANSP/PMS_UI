...
export default function TechnicianDashboard() {
  ...
  const {
    ...
    handleOpenDetails, handleCloseDetails, handleExecute, refetch,
  } = useDispenseQueue();
  ...
  const columns: Column<DispenseSummaryDto>[] = [
    {
      ...
      width: 160,
      render: (value) => (
        <span className="font-mono text-sm font-semibold text-blue-600 block truncate max-w-[140px]"
          title={String(value)}>
          {String(value)}
        </span>
      ),
      ...
      render: (value, row) => (
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 text-sm bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-200">
            ...
            <span className="text-gray-500">{Number(value) === 1 ? "item" : "items"} to pack</span>
          </div>
    ...
    },
    {
      ...
      width: 185,
      render: () => (
        <Badge label="Payment Processed" variant="default"
      ...
      render: (_, row) => {
        ...
        return (
          <div className="flex flex-row items-center gap-2" onClick={(e) => e.stopPropagation()}>
            ...
            <Button size="sm" variant="primary" onClick={() => handleExecute(row)}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        <KpiCard icon={<ClipboardList className="w-5 h-5 text-blue-600" />} iconBg="bg-blue-100"
          value={stats.paymentProcessed} label="Pending Dispense" sub="Payment processed — awaiting packing" />
        <KpiCard icon={<CheckCircle className="w-5 h-5 text-green-600" />} iconBg="bg-green-100"
          value={stats.dispensedToday} label="Dispensed Today" sub="Completed" />
        <KpiCard icon={<Package className="w-5 h-5 text-purple-600" />} iconBg="bg-purple-100"
          value={queueData.length} label="Queue Length" sub="Items to process" />
        <KpiCard icon={<AlertTriangle className="w-5 h-5 text-orange-600" />} iconBg="bg-orange-100"
          value={0} label="Inventory Alerts" sub="Needs attention"
          clickable onClick={() => navigate(ROUTES.TECHNICIAN.INVENTORY)} />
      </div>
        ...
        <div className="p-5 border-b border-gray-100">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">Pack and process prescriptions ready for patient pickup</p>
            </div>
            <div className="flex items-center gap-3">
              <div className="bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-200 flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-blue-500 animate-pulse" />
                <span className="text-sm font-medium text-blue-700">{stats.paymentProcessed} Pending</span>
              </div>
            </div>
          </div>
        </div>
...
interface KpiCardProps {
  ...
  label: string; sub: string; clickable?: boolean; onClick?: () => void;
}
function KpiCard({ icon, iconBg, value, label, sub, clickable, onClick }: KpiCardProps) {
  return (
    <div className={`bg-white rounded-xl border border-gray-200 p-5 shadow-sm ${clickable ? "cursor-pointer hover:shadow-md transition-shadow" : ""}`}
      onClick={onClick}>
